...
class SVSSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # -------------------------
        # Config toggles  (IMPORTANT: use cfg.train.use_stream)
        # -------------------------
        ...
        dbg = bool(getattr(self.cfg.train, "debug", False))
        ...
        # -------------------------
        # Query indices (SUPERVISED set, TOTAL frames)
        #   - train: sampled from allowed (disjoint from support)
        #   - val/test: evenly spaced or all
        # -------------------------
        if self.split == "train":
        ...
        # -------------------------
        # Enforce disjoint support/query + fixed length if needed
        # -------------------------
        ...
        # -------------------------
        # Debug prints (sampling only)
        # -------------------------
        if do_dbg:
            def _chunks(lst, k):
                return [lst[i:i+k] for i in range(0, len(lst), k)] if k > 0 else [lst]

            def _is_consecutive(win):
                return all(win[i+1] == win[i] + 1 for i in range(len(win)-1))

            print("\n================= SVSSDataset DEBUG =================")
            print(f"split={self.split} stream_query={stream_query} idx={idx}")
            print(f"clip: vs={vs} vc={vc} n_frames={n}")
            print(f"allowed_max_items={int(self.allowed_max_items)} allowed_size={int(len(allowed))}")
            print("allowed[:30] =", list(map(int, allowed[:30].tolist())))

            print(f"Kwin={Kwin}  S_total={len(s_idx)}  T_total={len(q_idx)}")
            print("support_idx[:40] =", list(map(int, s_idx[:40])))
            print("query_idx  [:40] =", list(map(int, q_idx[:40])))

            inter = sorted(set(map(int, s_idx)).intersection(set(map(int, q_idx))))
            print("support ∩ query =", inter)

            bad_s = [int(i) for i in s_idx if int(i) not in allowed_set]
            bad_q = [int(i) for i in q_idx if int(i) not in allowed_set]
            print("support_not_in_allowed =", bad_s)
            print("query_not_in_allowed   =", bad_q)

            if Kwin > 1:
                s_w = _chunks(list(map(int, s_idx)), Kwin)
                q_w = _chunks(list(map(int, q_idx)), Kwin)
                print("support_windows[:10] =", s_w[:10])
                print("query_windows  [:10] =", q_w[:10])

                if bool(getattr(self.cfg.train, "allowed_strict_consecutive", True)):
                    print("support_strict_consecutive[:10] =", [_is_consecutive(w) for w in s_w[:10]])
                    print("query_strict_consecutive  [:10] =", [_is_consecutive(w) for w in q_w[:10]])

        # -------------------------
        # Load supports (always small)
        # -------------------------
        sup_imgs, sup_msks = [], []
        sup_img_paths, sup_msk_paths = [], []
        for si in s_idx:
            p_img = g.loc[int(si), "img"]
            p_msk = g.loc[int(si), self.mask_col]
            sup_img_paths.append(p_img)
            sup_msk_paths.append(p_msk)
        ...
        if do_dbg:
            print("support_img_tensor =", tuple(sup_img_t.shape), "support_mask_tensor =", tuple(sup_msk_t.shape))
            print("support_img_paths[:3] =", sup_img_paths[:3])
            print("support_msk_paths[:3] =", sup_msk_paths[:3])

        # ============================================================
        # STREAMING MODE
        #   - rollout: full timeline as paths (1..n-1)
        #   - supervise: ONLY q_idx (t_query) frames
        # ============================================================
        if stream_query:
            # query_roll_idx = list(range(1, n))  # (T_rollout,)

            T_roll = int(getattr(self.cfg.train, "rollout_len", -1))
            mode   = str(getattr(self.cfg.train, "rollout_mode", "full"))
            ...
            if mode == "full" or T_roll < 0:
                ...
                rng = self._get_rng(idx + 100000 * int(self.epoch_salt))
                start = rng.randint(1, (n - T_roll)) if (n - T_roll) > 1 else 1
                query_roll_idx = list(range(start, start + T_roll))

            elif mode == "allowed":
                # rollout only through allowed frames (except 0)
            ...
            # supervised indices must be in rollout space [1..n-1]
            q_sup = [int(i) for i in q_idx if 1 <= int(i) <= (n - 1)]
            if len(q_sup) == 0:
                q_sup = [query_roll_idx[0]]

            q_img_paths = [str(g.iloc[i]["img"]) for i in query_roll_idx]
            q_msk_paths = [str(g.iloc[i][self.mask_col]) for i in query_roll_idx]

            if do_dbg:
                print("STREAMING enabled: not loading query tensors.")
                print("roll_idx[:10] =", query_roll_idx[:10])
                print("sup_idx       =", q_sup)
                print("query_img_paths[:3] =", q_img_paths[:3])
                print("query_msk_paths[:3] =", q_msk_paths[:3])
                print("====================================================\n")
            ...
            return {
                ...
                # tensors (so svss_collate_stream can torch.stack)
                "support_indices": torch.as_tensor(list(map(int, s_idx)), dtype=torch.long),     # (S,)
                "query_roll_indices": torch.as_tensor(query_roll_idx, dtype=torch.long),        # (T_rollout,)
                "query_indices": torch.as_tensor(q_sup, dtype=torch.long),                       # (t_query,)

                "query_img_paths": q_img_paths,   # list[str] length T_rollout
                "query_mask_paths": q_msk_paths,  # list[str] length T_rollout
        ...
        # -------------------------
        # NON-STREAMING: load query tensors (current behavior)
        # -------------------------
        q_imgs, q_msks = [], []
        for qi in q_idx:
        ...
        if do_dbg:
            print("NON-STREAMING: loaded query tensors.")
            print("query_img_tensor =", tuple(q_imgs_t.shape), "query_mask_tensor =", tuple(q_msks_t.shape))
            print("====================================================\n")

        return {
            "support_img": sup_img_t,  # (S,3,H,W)
            "support_mask": sup_msk_t, # (S,H,W)
            "support_indices": list(map(int, s_idx)),
            "query_imgs": q_imgs_t,    # (T,3,H,W)
            "query_masks": q_msks_t,   # (T,H,W)
            "query_indices": list(map(int, q_idx)),
            "video_src": vs,
            "video_clip": vc,
            "video_len": int(n),
        }
