# BACKUP FOR VERSION THAT SUPPORTED REGISTERS
...
class UKASTdb(nn.Module):
    ...
    def forward(self, x_in):
        ...
        if reg_tokens is not None:
            # reg_global = reg_tokens.mean(dim=1)        # [B, 192]
            # reg_global = self.reg_bottleneck_proj(reg_global)  # [B, 384]
            # reg_global = reg_global.view(
            #     reg_global.size(0), reg_global.size(1), *([1] * (dec4.ndim - 2))
            # )
            # dec4 = dec4 + reg_global
...
class RegisterFusion(nn.Module):
    def __init__(self, dim, reg_dim, num_heads=4):
        super().__init__()
        # Project registers into same dim as bottleneck
    ...
    def forward(self, dec4, reg_tokens):
        """
        dec4: [B, C, H, W]  (bottleneck feature map)
        reg_tokens: [B, R, reg_dim]  (register embeddings)
        """
        B, C, H, W = dec4.shape
        x = dec4.view(B, C, -1).transpose(1, 2)   # [B, HW, C]

        reg_proj = self.reg_proj(reg_tokens)      # [B, R, C]

        # Cross-attn: queries = dec4 spatial tokens, keys/values = registers
        fused, _ = self.cross_attn(query=x, key=reg_proj, value=reg_proj)

        fused = self.norm(x + fused)              # residual
