...
# from networks.rational_kat_cu.kat_rational import KAT_Group2D
from networks.rational_kat_cu.kat_rational import KAT_Group as KAT_Group2D
...
class UKAST(nn.Module):
    ...
    def forward(self, x_in):
        ...
        hidden_states_out = self.swinKAT(x_in, self.normalize)
...
class SwinKATBlock(nn.Module):
    ...
    def forward_part2(self, x):
        ...
        x = self.norm2(x)  # LayerNorm over C
        
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)   # -> [B, L, C], L = H*W
        
        # apply MLP (KAN with Linear)
        x = self.mlp(x)           # still [B, L, C]
