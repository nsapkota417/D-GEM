...
class UKAT(nn.Module):
    ...
    def __init__(
        ...
        Examples::
            ...
            # for single channel input 4-channel output with image size of (512,512), feature size of 32
            >>> net = UKAT(in_channels=1, out_channels=4, img_size=(512,512), feature_size=32, norm_name='batch')

            # for 3-channel input 2-channel output with image size of (224,224), feature size of 16
            >>> net = UKAT(in_channels=3, out_channels=2, img_size=(224,224), feature_size=16, norm_name='instance')
        ...
        self.kat = KATVisionTransformer(
            ...
            spatial_dims=spatial_dims,  # Enable 2D/3D support
            proj_type=proj_type,
    ...
    def forward(self, x_in):
        ...
        x, hidden_states_out = self.kat.forward_intermediates(
            x_in, 
            indices=[2, 5, 8, 11],  # layers 3, 6, 9, 12 (0-indexed)
            return_prefix_tokens=False,
            norm=False,
            stop_early=False,
            output_fmt='NLC',  # Keep as sequence format
            intermediates_only=False
        )

        # Extract intermediate layers (convert to dict format like original UNETR)
        hidden_states_dict = {
            3: hidden_states_out[0],   # layer 3
            6: hidden_states_out[1],   # layer 6  
            9: hidden_states_out[2],   # layer 9
        }

        # UNETR forward pass with KAT features
        enc1 = self.encoder1(x_in)
        
        x2 = hidden_states_dict[3]
        enc2 = self.encoder2(self.proj_feat(x2))
        
        x3 = hidden_states_dict[6]
        enc3 = self.encoder3(self.proj_feat(x3))
        
        x4 = hidden_states_dict[9]
