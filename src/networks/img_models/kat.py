...
# MONAI compatibility imports (optional)
try:
...
from networks.rational_kat_cu.kat_rational import KAT_Group
...
class KATVisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            act_init: str = 'gelu',
            spatial_dims: int = 2,  # Add spatial_dims parameter for MONAI compatibility
            proj_type: str = 'conv',  # Add proj_type for MONAI PatchEmbeddingBlock
            pos_embed_type: str = 'learnable',  # Add pos_embed_type for MONAI PatchEmbeddingBlock
    ) -> None:
        """
        Args:
            ...
            spatial_dims: Number of spatial dimensions (2 for 2D, 3 for 3D). Used with MONAI compatibility.
            proj_type: Patch embedding projection type for MONAI PatchEmbeddingBlock.
            pos_embed_type: Position embedding type for MONAI PatchEmbeddingBlock.
        """
        super().__init__()
        
        # Validate spatial dimensions for MONAI compatibility
        if spatial_dims not in (2, 3):
            raise ValueError("spatial_dims must be 2 or 3 when using MONAI PatchEmbeddingBlock")
        
        if not MONAI_AVAILABLE:
            raise ImportError("MONAI is required when use_monai_patch_embed=True. Please install MONAI.")
        ...
        # Handle image size and patch size for MONAI compatibility
        if ensure_tuple_rep is not None:
        ...
        # Choose patch embedding layer - Always use MONAI PatchEmbeddingBlock
        self.patch_embed = PatchEmbeddingBlock(
            ...
            dropout_rate=pos_drop_rate,
            spatial_dims=spatial_dims,
        )
        num_patches = self.patch_embed.n_patches
        reduction = patch_size[0] if isinstance(patch_size, (list, tuple)) else patch_size
        
        # # Original timm PatchEmbed (commented out but kept for reference)
        ...
        # Position embedding handling - MONAI PatchEmbeddingBlock handles position embedding internally
        self.pos_embed = None
        # # Original timm behavior for position embedding (commented out)
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        # MONAI PatchEmbeddingBlock already handles position embedding
        # We just need to add class and register tokens if they exist
        to_cat = []
        if self.cls_token is not None:
        ...
        # # Original timm behavior (commented out but kept for reference)
    ...
    def forward_intermediates(
        ...
        B = x.shape[0]
        _, _, H, W = x.shape  # keep spatial for reshaping later
        ...
        # --- concat registers here ---
        if registers is not None:
            x = torch.cat([x, registers], dim=1)  # now works: both [B, N, C]
        ...
        # strip prefix tokens for intermediates
