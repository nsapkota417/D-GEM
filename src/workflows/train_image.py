...
def summarize(df):
    cols = [c for c in df.columns if c != "video_src"]
    return (
        df.groupby("video_src")[cols]
        .agg(lambda s: ((s.fillna("").astype(str).str.strip() != "") &
                        (s.fillna("").astype(str).str.strip() != "-")).sum())
        .reset_index()
    )
...
if not cfg.val.eval_only:
    ...
    if cfg.data.use_half:
        train_df = (
            pd.concat([
                ...
                train_df[train_df["rep"] == "rep"],  # always keep representative rows
            ])
            .drop_duplicates()
            .reset_index(drop=True)
        )
    ...
    with banner():
        print(f"Train videos ({train_df.video_src.nunique()}):")
        print("  " + ", ".join(sorted(train_df.video_src.unique())))
        print()

        print(f"Val videos ({test_df.video_src.nunique()}):")
        print("  " + ", ".join(sorted(test_df.video_src.unique())))
        print()

        print("-" * 80)
        print("Train summary")
        print("-" * 80)
        print(summarize(train_df).to_string(index=False))

        print()
        print("-" * 80)
        print("Validation summary")
        print("-" * 80)
        print(summarize(test_df).to_string(index=False))

        print(f'Using Column: {cfg.data.mask_col}')
...
if cfg.val.eval_only:
    ...
    with banner():
        ...
        print(f"Inf videos ({inf_df.video_src.nunique()}):")
        print("  " + ", ".join(sorted(inf_df.video_src.unique())))
        print()
        print("-" * 80)
        print("Inference summary")
        print("-" * 80)
        print(summarize(inf_df).to_string(index=False))

        print(f'Using Column: {cfg.data.mask_col}')
        print(f"Inference samples   : {len(inf_df):,}")
        print()
    ...
    with torch.inference_mode():
        results = inference_with_miou(
            ...
            device=device,
            save_dir=res_dir,
            save_outputs=cfg.val.save_outputs,
        )
