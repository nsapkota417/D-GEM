...
class Memory(nn.Module):
    ...
    def forward(self, m_in, m_out, q_in, q_out):  # m_in: o,c,t,h,w
        ...
        mi = m_in.reshape(m_in.shape[0], m_in.shape[1], -1)
        
        mi = torch.transpose(mi, 1, 2)  # b, THW, emb
 

        # --- robust query reshape (and optional resize to match memory spatial) ---
        Bq, De, Hq, Wq = q_in.shape  # trust q_in, not external H/W

        # If STM computed H/W from memory and it doesn't match q_in, force match.
        # (This avoids shape bugs when memory/query spatial sizes differ.)
        if 'H' in locals() and 'W' in locals():
            Hm, Wm = int(H), int(W)
            if (Hq != Hm) or (Wq != Wm):
                q_in = F.interpolate(q_in, size=(Hm, Wm), mode="bilinear", align_corners=False)
                Bq, De, Hq, Wq = q_in.shape

        qi = q_in.reshape(Bq, De, Hq * Wq) 
